                                        ...
                                        @foreach ($wo_data as $key => $row)
                                            @php
                                                ...
                                                $tenderdata = $tendername
                                                    ->where('id', $matchedData->tender_name_id)
                                                    ->first();
                                            @endphp
                                            <tr>
                                                <td>{{ $key + 1 }}</td>
                                                <td>{{ $matchedData ? \Carbon\Carbon::parse($matchedData->date)->format('d-m-Y') : '' }}
                                                </td>
                                                <td>{{ $tenderdata ? Str::limit($tenderdata->tender_name, 20) : 'N/A' }}
                                                </td>
                                                ...
                                                <td>
                                                    ...
                                                    <a href="{{ asset('admin/woviewbuttenfoa/' . Crypt::encrypt($row->id)) }}"
                                                        class="btn btn-warning btn-sm btn-icon">View Button</a>
                                                    <a href="" class="btn btn-warning btn-sm btn-icon">WO Details
                                                        Form</a><br>
                                                    <a href="{{ asset('admin/wodetailupdate/' . Crypt::encrypt($row->id)) }}"
                                                        class="mt-1 btn btn-warning btn-sm btn-icon ">
                                                        WO Uplode Form
                                                    </a>

                                                    <a href="" class=" mt-1 btn btn-warning btn-sm btn-icon">WO
                                                        Acceptance(for TL, CEO only)</a>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
