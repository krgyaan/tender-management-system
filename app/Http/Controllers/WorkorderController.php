...
class WorkorderController extends Controller
{
    ...
    public function basicdetailadd($id)
    {
        $tender = TenderInfo::select('id', 'tender_name')->findOrFail($id);
        // dd($id, $tender->toArray());
        return view('basicdetails.basicdetailadd', compact('tender'));
    }
    ...
    public function basicdetailupdatepost(Request $request)
    {
        ...
        $data->par_amt = $request->pre_amt;
        if ($request->image) {
            $img = time() . 'basicdetails.' . $request->image->extension();
            $request->image->move(public_path('upload/basicdetails'), $img);
            $data->image = $img;
        }
        $data->save();
    ...
    {
        $tender = TenderInfo::with('basic_details')->select('id', 'tender_name', 'organisation')->findOrFail($id);
        // dd($id, $tender);
        return view('wodetails.wodetailadd', compact('tender'));
    }
    ...
    public function wodetailaddpost(Request $request)
    {
        ...
        if ($this->workorder_mail($productprice->basic_detail_id)) {
            ...
            return redirect()->route('basicdetailview')->with('success', 'WO Details saved successfully.');
        } else {
    ...
    public function woacceptanceform($id)
    {
        ...
        return view('wodetails.woacceptanceform', compact('basic'));
    }
