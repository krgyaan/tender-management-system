...
class TlPerformanceController extends Controller
{
    ...
    public function performance(Request $request)
    {
        ...
        if ($request->method() == 'GET') {
            $result = false;
            return view('performance.tl', compact('users', 'result'));
        }
        ...
        if ($request->method() == 'POST') {
            ...
            Log::info("Step 1: Getting Team Leader ID: $team_member");
            ...
            $bidedTenders = DB::table('bid_submissions as bs')
                ...
                ->when($from && $to, function ($query) use ($from, $to, $user_team) {
                    $query->where(function ($q) use ($from, $to, $user_team) {
                        $q->where(function ($sub) use ($from, $to, $user_team) {
                            $sub->where('bs.status', 'Bid Submitted')
                                ->whereBetween('bs.bid_submissions_date', [$from, $to])
                                ->where('ti.team', $user_team);
                        })
                            ->orWhere(function ($sub) use ($from, $to, $user_team) {
    ...
    private function classifyBidedTenders(Collection $tenders): array
    {
        ...
        foreach ($tenders as $tender) {
            ...
            $results['bid_tender']['value'] += $tender->gst_values;
            ...
            foreach ($statusGroups as $label => $statuses) {
                if (in_array($tender->status, $statuses)) {
                    $results[$label]['count']++;
                    $results[$label]['value'] += $tender->gst_values;
    ...
    private function parseEmdDetailsFromCollection($tender, $emdsByTenderNo): array
    {
        ...
        switch ($type) {
            ...
            case 2: // FDR
                ...
                if ($fdr) $result['emds'] += $fdr->fdr_amt;
                break;
            ...
            $performanceData['tender_wise_details'][$tenderId] = [
                ...
                'stage_timelines' => array_map(
                    fn($stage) => [
                        ...
                        'on_time' => in_array($stage, array_keys($stageData['stages'] ?? [])),
                    ],
                    $roleStages
                )
            ];
