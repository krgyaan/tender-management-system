<div>
    @php
        $basicDetails = App\Models\Basic_detail::whereIn('id', array_column($wodata, 'basic_detail_id'))
            ->get()
            ->keyBy('id');
    @endphp
    ...
    @foreach ($wodata as $item)
        ...
        <div style="padding: 0px 30px">
            <h3>Dear Sir ,</h3>
            <p style="margin-top: -13px;"> We thank you for placing your trust in us and releasing the WO no.
                {{ $basicdata->number ?? 'N/A' }}, Date {{ $basicdata->date ?? 'N/A' }} .</p>
            <p style=""> After carefully reviewing the order, we identified a few clauses requiring amendment.
            ...
            <tbody>
                @if (is_object($item) && isset($item->page_no))
                    @php
                        ...
                        $clauseNos = is_string($item->clause_no)
                            ? json_decode($item->clause_no, true)
                            : [$item->clause_no];
                        if (!is_array($clauseNos)) {
                            $clauseNos = explode(',', $item->clause_no);
                        }

                        $currentStatements = is_string($item->current_statement)
                            ? json_decode($item->current_statement, true)
                            : [$item->current_statement];
                        if (!is_array($currentStatements)) {
                            $currentStatements = explode(',', $item->current_statement);
                        }

                        $correctedStatements = is_string($item->corrected_statement)
                            ? json_decode($item->corrected_statement, true)
                            : [$item->corrected_statement];
