    ...
    <section>
        ...
        <div class="row mt-4">
            <div class="col-md-12 pb-3">
                <form action="{{ route('dateFilter') }}" method="POST" class="row g-3">
                    @csrf
            ...
            <div class="col-md-12 m-auto">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('payment-history', Auth::user()->id) }}" class="btn btn-secondary btn-sm">Payment
                        History</a>
                    <a href="{{ route('imprest-voucher', Auth::user()->id) }}"
                        class="btn btn-sm btn-outline-success">Imprest Voucher</a>
                                ...
                                <tbody>
                                    @if ($employeeimprest)
                                        @foreach ($employeeimprest as $key => $employeeimprest)
                                            @if (
                                                $employeeimprest->name_id == Auth::user()->id ||
                                                    $employeeimprest->team_id == Auth::user()->id ||
                                                    in_array(Auth::user()->role, ['admin', 'account']))
                                                <tr>
                                                    <td>
                                                        {{ \Carbon\Carbon::parse($employeeimprest->created_at)->format('Y-m-d') }}
                                                    </td>
                                                    ...
                                                    <td>
                                                        {{ $employeeimprest->acc_remark }}<br>
                                                        @if ($employeeimprest->buttonstatus == 1)
                                                            <br>Approved on {{ date('d M, Y', strtotime($employeeimprest->approved_date)) }}
                                                        @endif
                                                    </td>
                                                    <td>
                                                        @if ($employeeimprest->buttonstatus == 1)
