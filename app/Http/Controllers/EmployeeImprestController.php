    ...
    public function adminSignedVoucher(Request $request, $id)
    {
        ...
        try {
            ...
            if ($user->role == 'admin') {
                ...
                Log::info('Voucher signed successfully by admin', ['voucher' => $voucher]);
                return response()->json(['success' => true, 'message' => 'Voucher signed successfully']);
            } else {
    ...
    public function accountSignedVoucher(Request $request, $id)
    {
        ...
        try {
            ...
            if ($user->role == 'accountant') {
                ...
                $voucher->save();
                Log::info('Voucher signed successfully by account', ['voucher' => $voucher]);
            } else {
                Log::warning('Unauthorized attempt to sign voucher', ['user_id' => $user->id, 'voucher_id' => $id]);
                return response()->json(['success' => false, 'message' => 'You are not authorized to sign the voucher']);
            }
