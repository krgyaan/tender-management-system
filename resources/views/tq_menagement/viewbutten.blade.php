@extends('layouts.app')
@section('page-title', 'TQ Management View')
        ...
        <div class="col-md-12">
            <div class="card m-auto">
                <div class="card-body" id="print">
                    ...
                    <table class="table-bordered w-100 my-4">
                        <thead>
                            <tr>
                                <th>Costing Sheet</th>
                                <th>
                                    <a href="{{ $tender->sheet->driveid }}" target="_blank">Open Costing Sheet</a>
                                </th>
                            </tr>
                        </thead>
                    </table>
                                        ...
                                        @if ($rfq->scopes)
                                            @foreach ($rfq->scopes as $item)
                                                <a href="{{ asset('uploads/rfqdocs/' . $item->file_path) }}"
                                                    target="_blank" rel="noopener noreferrer">Scope of Work - {{ $loop->iteration }}</a>
                                            @endforeach
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        @if ($rfq->technicals)
                                            @foreach ($rfq->technicals as $item)
                                                <a href="{{ asset('uploads/rfqdocs/' . $item->file_path) }}"
                                                    target="_blank" rel="noopener noreferrer">Technical Specifications - {{ $loop->iteration }}</a>
                                            @endforeach
                                        @endif
                                    </td>
                                </tr>
                                    ...
                                    <td>
                                        @if ($rfq->boqs)
                                            @foreach ($rfq->boqs as $item)
                                                <a href="{{ asset('uploads/rfqdocs/' . $item->file_path) }}"
                                                    target="_blank" rel="noopener noreferrer">Detailed BOQ - {{ $loop->iteration }}</a>
                                            @endforeach
                                        @endif
                                    </td>
                                    <th>MAF Format</th>
                                    <td>
                                        @if ($rfq->mafs)
                                            @foreach ($rfq->mafs as $item)
                                                <a href="{{ asset('uploads/rfqdocs/' . $item->file_path) }}"
                                                    target="_blank" rel="noopener noreferrer">MAF Format - {{ $loop->iteration }}</a>
                                            @endforeach
                                        @endif
                                    </td>
                                </tr>
                                    ...
                                    <td>
                                        @if ($rfq->miis)
                                            @foreach ($rfq->miis as $item)
                                                <a href="{{ asset('uploads/rfqdocs/' . $item->file_path) }}"
                                                    target="_blank" rel="noopener noreferrer">MII Format - {{ $loop->iteration }}</a>
                                            @endforeach
                                        @endif
                                    </td>
                            ...
                            <table class="table-bordered w-100 mt-2">
                                <tbody>
                                    <tr>
                                        ...
                                        <td>
                                            @if ($tender->bs->bid_submission_date)
                                                {{ \Carbon\Carbon::parse($tender->bs->bid_submission_date)->format('d-m-Y h:i A') }}
                                            @else
                                                —
                                            @endif
                                        </td>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($tender->bs->reason_for_missing)
                                                {!! nl2br(e(wordwrap($tender->bs->reason_for_missing, 40, "\n"))) !!}
                                            @else
                                                —
                                            @endif
                                        </td>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($tender->bs->not_repeat_reason)
                                                {!! nl2br(e(wordwrap($tender->bs->not_repeat_reason, 40, "\n"))) !!}
                                            @else
                                                —
                                            @endif
                                        </td>
                                        ...
                                        <td>
                                            @if ($tender->bs->tms_improvements)
                                                {!! nl2br(e(wordwrap($tender->bs->tms_improvements, 40, "\n"))) !!}
                                            @else
                                                —
                                            @endif
                                        </td>
                    ...
                    @endif
                    <div class="tq_received mt-5">
                        <div class="text-center fw-bold">
                            <h5 class="mt-5 mb-0">TQ Management Data</h5>
                            </div>
                        @if($tender->tq_received != [])
                            @foreach($tender->tq_received as $tqrec)
                                @php
                                    $types = json_decode($tqrec->tq_type ?? '[]', true);
                                    $descs = json_decode($tqrec->description ?? '[]', true);
                                @endphp
                                ...
                                <div class="mt-4 border p-3">
                                        ...
                                        <table class="table-bordered w-100 mb-3">
                                            ...
                                            <tbody>
                                                @foreach ($types as $index => $type)
                                                    <tr>
                                                        <td>{{ $loop->iteration }}</td>
                                                        <td>{{ \App\Models\Tq_type::findOrFail($type)->tq_type }}</td>
                                                        <td>{{ $descs[$index] ?? '—' }}</td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                    ...
                                    <table class="table-bordered w-100">
                                        <tbody>
                                            <tr>
                                                <th style="width: 25%">TQ Submission Date</th>
                                                <td style="width: 25%">{{ date('d-m-Y', strtotime($tqrec->tq_submission_date)) }}</td>
                                                <th style="width: 25%">TQ Submission Time</th>
                                                <td style="width: 25%">{{ date('h:i A', strtotime($tqrec->tq_submission_time)) }}</td>
                                            </tr>
                                            <tr>
                                                <th>TQ Document</th>
                                                <td colspan="3">
                                                    @if($tqrec->tq_document)
                                                        <a href="{{ asset('uploads/tq/' . $tqrec->tq_document) }}" target="_blank">
                                                            View File        
                                                        </a>
                                                    @else
                                                        Not Uploaded
                            ...
                            @endforeach
                        @endif
                        @if($tender->tq_replied != [])
                            @foreach($tender->tq_replied as $tqrep)
                                <div class="mt-4 border p-3">
                                    <table class="table-bordered w-100 mt-2">
                                        <tbody>
                                            <tr>
                                                <th>TQ Submissions Date</th>
                                                <td>{{ date('d-m-Y', strtotime($tqrep->tq_submission_date)) }}</td>
                                                <th>TQ Submissions Time</th>
                                                <td>{{ date('h:i A', strtotime($tqrep->tq_submission_time)) }}</td>
                                            </tr>
                                            <tr>
                                                <th>TQ Document</th>
                                                <td>
                                                    @if($tqrep->tq_document)
                                                        <a href="{{ asset('uploads/tq/' . $tqrep->tq_document) }}" target="_blank">
                                                            View File        
                                                        </a>
                                                    @else
                                                        Not Uploaded
                                                ...
                                                <th>Proof of Submission</th>
                                                <td>
                                                    @if($tqrep->proof_submission)
                                                        <a href="{{ asset('uploads/tq/' . $tqrep->proof_submission) }}" target="_blank">
                                                            View File        
                                                        </a>
                                                    @else
                                                        Not Uploaded
                        ...
                        @endif
                        @if($tender->tq_missed != [])
                            @foreach($tender->tq_missed as $tqmiss)
                                <div class="mt-4 border p-3">
                                    <table class="table-bordered w-100 mt-2">
                                        <tbody>
                                            <tr>
                                                <th>Reason of Missing</th>
                                                <tdcolspan="3">{{ $tqmiss->reason_missing }}</td>
                                            </tr>
                                            <tr>
                                                <th>Would Ensure not Repeated</th>
                                                <tdcolspan="3">{{ $tqmiss->would_repeated }}</td>
                                            </tr>
                                            <tr>
                                                <th>Improvements Needed TMS System</th>
                                                <tdcolspan="3">{{ $tqmiss->tms_system }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
