...
class EmdDashboardController extends Controller
{
        ...
        if ($request->isMethod('get')) {
            try {
                ...
                $data['emdPopPending'] = PayOnPortal::with(['emd', 'emd.tender', 'emd.tender.statuses'])
                    ->whereNull('action')
                    ->get()
                    ->sortBy(fn($item) => optional($item->emd)->due_date
                        ? Carbon::parse($item->emd->due_date)
                        : now()->addYears(100))
                    ->values(); // reindex after sort

                // 2. Done – Sort by parsed date_time (which is a string in d-m-Y H:i:s)
                $data['emdPopDone'] = PayOnPortal::with(['emd', 'emd.tender', 'emd.tender.statuses'])
                    ->whereNotNull('action')
                    ->get()
    ...
    public function bgRejectedMail($id)
    {
        try {
            $bg = EmdBg::find($id);
            $to = User::where('name', 'LIKE', $bg->emds->requested_by . '%')->first();
            $ccRoles = User::where('team', $to->team)
