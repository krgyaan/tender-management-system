...
function startCountdown(timerElement) {
    ...
    function updateTimer() {
        const isNegative = countdown < 0;
        const absCountdown = Math.abs(countdown);

        const hours = Math.floor(absCountdown / 3600);
        const minutes = Math.floor((absCountdown % 3600) / 60);
        const seconds = absCountdown % 60;

        // Format as HH:MM:SS
        timerElement.textContent =
            `${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;

        // Change color based on whether the countdown is negative
        if (!isNegative) {
...
const wordwrap = (str, width = 40) =>
  str.replace(new RegExp(`(.{1,${width}})(\\s+|$)|(.{1,${width}})`, 'g'), '$&\n');
