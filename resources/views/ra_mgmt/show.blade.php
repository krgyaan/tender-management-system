@extends('layouts.app')
@section('page-title', 'RA Management View')
        ...
        <div class="col-md-12">
            <div class="card m-auto">
                <div class="card-body" id="print">
                    ...
                    @endif
                    <div class="ra_mgmt mt-5">
                        <div class="text-center fw-bold">
                            <h5 class="mt-5 mb-0">RA Management Data</h5>
                        </div>
                        @if($tender->ra_mgmt)
                            @foreach($tender->ra_mgmt as $ra)
                                <div class="mt-4 border p-3">
                                    <table class="table-bordered w-100">
                                        <tbody>
                                            <tr>
                                                <th>No. of Qualified Parties</th>
                                                <td>{{ $ra->qualified_parties }}</td>
                                                <th>Names of Qualified Parties</th>
                                                <td></td>
                                            </tr>
                                            <tr>
                                                <th>Screenshot of Qualified Parties</th>
                                                <td>
                                                    @if($ra->screenshot_qualified_parties)
                                                        <a href="{{ asset('ra_results/' . $ra->screenshot_qualified_parties) }}" target="_blank">
                                                            View File        
                                                        </a>
                                                    @else
                                                        Not Uploaded
                                                ...
                                                @php
                                                    $result = 0;
                                                    if ($ra->start_price > 0) {
                                                        $result = (($ra->start_price - $ra->close_price) / $ra->start_price) * 100;
                                                    }
                                                @endphp
                                                
                                                <th>RA Result (%age Below the Start price)</th>
                                                <td>{{ number_format($result, 2) }}%</td>
                                            </tr>
                                            <tr>
                                                @php
                                                    $start = Carbon::parse($ra->start_time);
                                                    $close = Carbon::parse($ra->close_time);
                                                    $duration = $close->diff($start)->format('%H:%I:%S');
                                                @endphp
                                                <th>RA Duration (RA Close time - RA Start time)</th>
                                                <td>
                                                    {{ $duration }}
                                                </td>
                                                <th>Screenshot of Decrements</th>
                                                <td>
                                                    @if($ra->screenshot_decrements)
                                                        <a href="{{ asset('ra_results/' . $ra->screenshot_decrements) }}" target="_blank">
                                                            View File        
                                                        </a>
                                                    @else
                                                        Not Uploaded
                                            ...
                                            <tr>
                                                <th>Screenshot of the Final result</th>
                                                <td colspan="3">
                                                    @if($ra->final_result_screenshot)
                                                        <a href="{{ asset('ra_results/' . $ra->final_result_screenshot) }}" target="_blank">
                                                            View File        
                                                        </a>
                                                    @else
                                                        Not Uploaded
