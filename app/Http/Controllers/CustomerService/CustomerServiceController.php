...
class CustomerServiceController extends Controller
{
    ...
    public function getCustomerComplaintsData(Request $request)
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('service_engineer', function ($complaint) {
            ...
            ->rawColumns(['customer', 'project', 'attachment', 'action', 'timer' ,'service_engineer'])
            ->make(true);
    }
    ...
    {
        $prefix = "TKT";
        $date   = now()->format('Ymd'); // e.g. 20250829

        // Count how many complaints exist today
        $countToday = CustomerComplaint::whereDate('created_at', now()->toDateString())->count() + 1;

        // Pad with 2 digits (01, 02, 03...)
    ...
    public function allotServiceEngineer(Request $request)  {
        ...
        if ($this->mailToServiceEngineer($complaint->id, $data)) {
    ...
    public function mailToServiceEngineer($id ,$data)
    {
        try {
            $complaint = CustomerComplaint::findOrFail($id);
            // dd($complaint);
