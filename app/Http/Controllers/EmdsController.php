...
use App\Mail\PopTenderFeeMail;
use App\Mail\DdTenderFeeMail;
use App\Mail\PopCreatedMail;
use App\Models\BankTransfer;
use Illuminate\Http\Request;
use App\Mail\BankTransferMail;
use App\Mail\BtTenderFeeMail;
...
use App\Services\EmdResponsibilityService;
...
class EmdsController extends Controller
{
    ...
    protected function cc($team)
    {
        $users = User::where('team', $team)
            ->whereIn('role', ['admin', 'coordinator', 'team-leader'])
            ->pluck('email', 'role')
            ->toArray();
        return array_merge($users, ['accounts@volksenergie.in']);
    }
    ...
    public function emdData(Request $request, $type)
    {
        ...
        $query = TenderInfo::with(['emds', 'users', 'info:emd_req'])
        ...
        return DataTables::of($query)
            ...
            ->addColumn('action', function ($tender) {
                return view('partials.emd-action', ['tender' => $tender, 'bi_types' => $this->instrumentType])->render();
            })
    ...
    {
        try {
            if ($request->instrument_type != 1) {
                return redirect()->route('emds.index')->with('error', 'Invalid instrument type.');
            }
            if ($request->method() === 'GET') {
                $id = base64_decode($request->tender_no);
                if (!$id) {
                    throw new \Exception('Invalid tender number.');
                }
                $data['tender'] = TenderInfo::where('id', $id)
                    ->where('tlStatus', '1')
                    ->where('deleteStatus', '0')->first();
                if (!$data['tender']) {
                    throw new \Exception('Tender not found.');
                }
                $data['instrumentType'] = $this->instrumentType;
                $data['instrument_type'] = $request->instrument_type;
                $data['dd_purposes'] = $this->dd_purposes;
                return view('emds.create-dd', $data);
                ...
                try {
                    $request->validate([
                        ...
                        'instrument_type' => 'required|in:1',
                        'requested_by' => 'required',
                        'dd_favour' => 'required',
                        'dd_amt' => 'required|numeric',
                        'dd_payable' => 'required',
                        'dd_purpose' => 'required',
                        'dd_needs' => ($request->tender_id == '00') ? 'nullable' : 'required',
                        'courier_add' => ($request->tender_id == '00') ? 'nullable' : 'required',
                        'courier_deadline' => ($request->tender_id == '00') ? 'nullable' : 'required',
                        'dd_date' => ($request->tender_id == '00') ? 'required|date' : 'nullable|date',
                        'remarks' => ($request->tender_id == '00') ? 'required' : 'nullable',
                        'tf_yn' => 'nullable',
                    ]);
                    $emdDetails = $request->only(['tender_id', 'tender_no', 'due_date', 'project_name', 'instrument_type', 'requested_by']);
                    $emd = $this->storeBasic($emdDetails);

                    // Store Demand Draft details
                    ...
                    $chq->save();
                    // If Tender Fee Applicable
                    if ($request->tf_yn == 'yes') {
                        $tender = null;
                        if ($emd->tender_id != '00') {
                            $tender = TenderInfo::where('id', $emd->tender_id)->first();
                        }
                        $this->createDdTenderFee($request, $emd, $tender);
                    }
                    // Update tender status if needed
                    if ($emd->tender_id != '00') {
                        $tender = TenderInfo::where('tender_no', $emd->tender_no)->first();
                        if ($tender) {
                            $tender->update(['status' => '5']);
                        }
                    }
                    // Timer, PDF, Email logic
                    $dd = [
                        'cheque_date' => date('d-m-Y'),
                        'cheque_amt' => $request->dd_amt,
                        'cheque_favour' => $request->dd_favour,
                    ];
                    ...
                    // Send mail
                    $mailSent = $this->ddMail($emd->id, $pdfFiles);
                    DB::commit();
                    if ($mailSent) {
                    ...
                    DB::rollBack();
                    return redirect()->route('emds.index')->with('error', $e->getMessage());
                }
            } else {
                throw new \Exception('Unsupported request method.');
            }
        } catch (\Exception $e) {
            Log::error('requestDd() exception: ' . $e->getMessage());
            return redirect()->route('emds.index')->with('error', $e->getMessage());
        }
    }

    public function requestFdr(Request $request)
    {
        try {
            if ($request->instrument_type != 2) {
            ...
            if ($request->method() == 'GET') {
                return view('emds.create-fdr', $data);
            }
            if ($request->method() == 'POST') {
                ...
                try {
                    $request->validate([
                        ...
                        'fdr_date' => ($request->tender_id == '00') ? 'required|date' : 'nullable|date',
                    ]);
                    $emdDetails = $request->only(['tender_id', 'tender_no', 'due_date', 'project_name', 'instrument_type', 'requested_by']);
                    $emd = $this->storeBasic($emdDetails);
                    $fdr = new EmdFdr();
                    ...
                    // $this->timerService->startTimer($fdr, 'fdr_ac_form', $hrs); // Optionally add timer logic
            ...
            Log::error('requestFdr() exception: ' . $e->getMessage());
            return redirect()->route('emds.index')->with('error', $e->getMessage());
        }
    }

    public function requestChq(Request $request)
    {
        try {
            if ($request->instrument_type != 3) {
                throw new \Exception('Something went wrong!');
            }
            if ($request->method() == 'GET') {
                $id = base64_decode($request->tender_no);
                ...
                return view('emds.create-chq', $data);
            }
            if ($request->method() == 'POST') {
                DB::beginTransaction();
                try {
                    ...
                    // Send mail
                    $mailSent = $this->chequeMail($emd->id, $pdfFiles);

                    DB::commit();
                    if ($mailSent) {
                        return redirect()->route('emds.index')->with('success', 'Cheque EMD created and Mail sent successfully');
                    } else {
                        return redirect()->route('emds.index')->with('success', 'Cheque EMD created but Mail not sent.');
                    }
                } catch (\Throwable $e) {
                    DB::rollBack();
                    return redirect()->route('emds.index')->with('error', $e->getMessage());
            ...
            Log::error('requestChq() exception: ' . $e->getMessage());
            return redirect()->route('emds.index')->with('error', $e->getMessage());
        }
    }

    public function requestBg(Request $request)
    {
        try {
            if ($request->instrument_type != '4') {
            ...
            if ($request->method() == 'GET') {
                ...
                $data['instrument_type'] = $request->instrument_type;
                $data['banks'] = $this->banks ?? [];
                $data['purpose'] = $this->bg_purpose ?? [];

                return view('emds.create-bg', $data);
            }
            if ($request->method() == 'POST') {
                DB::beginTransaction();
                try {
                    ...
                    $bg->generated_pdfs = json_encode($pdfFiles);
                    $bg->save();

                    // $this->timerService->startTimer($bg, 'bg_acc_form', $hrs); // Optionally add timer logic
                    ...
                    // Send mail
                    $mailSent = $this->bgCreatedMail($bg->id, $pdfFiles);
            ...
            Log::error('requestBg() exception: ' . $e->getMessage());
            return redirect()->route('emds.index')->with('error', $e->getMessage());
        }
    }

    public function requestBt(Request $request)
    {
        try {
            if ($request->instrument_type != '5') {
                throw new \Exception('Invalid instrument type.');
            }
            if ($request->method() == 'GET') {
                $id = base64_decode($request->tender_no);
                ...
                $data['instrumentType'] = $this->instrumentType ?? [];
                $data['instrument_type'] = $request->instrument_type;
                return view('emds.create-bt', $data);
            } elseif ($request->method() == 'POST') {
                Log::info('Creating Bank Transfer for EMD, user: ' . Auth::user()->name);
                ...
                try {
                    $request->validate([
                        ...
                        'bt_amount' => 'required',
                        'tf_consent' => 'nullable',
                        'tf_amount' => in_array($request->tf_consent, ['2', '1']) ? 'required' : 'nullable',
                        'tf_purpose' => in_array($request->tf_consent, ['2', '1']) ? 'required' : 'nullable',
                        'tf_bt_acc_name' => $request->tf_consent == '2' ? 'required' : 'nullable',
                        'tf_bt_acc' => $request->tf_consent == '2' ? 'required' : 'nullable',
                        'tf_bt_ifsc' => $request->tf_consent == '2' ? 'required' : 'nullable',
                        'tf_portal' => $request->tf_consent == '1' ? 'required' : 'nullable',
                        'tf_is_netbanking' => $request->tf_consent == '1' ? 'required' : 'nullable',
                        'tf_is_debit' => $request->tf_consent == '1' ? 'required' : 'nullable',
                        'tf_dd_needs' => $request->tf_consent == '3' ? 'required' : 'nullable',
                        'tf_purpose_of_dd' => $request->tf_consent == '3' ? 'required' : 'nullable',
                        'tf_in_favour_of' => $request->tf_consent == '3' ? 'required' : 'nullable',
                        'tf_dd_payable_at' => $request->tf_consent == '3' ? 'required' : 'nullable',
                        'tf_courier_address' => $request->tf_consent == '3' ? 'required' : 'nullable',
                        'tf_courier_deadline' => $request->tf_consent == '3' ? 'required' : 'nullable',
                    ]);

                    $emdDetails = $request->only(['tender_id', 'tender_no', 'due_date', 'project_name', 'instrument_type', 'requested_by']);
                    $emd = $this->storeBasic($emdDetails);
                    Log::info('EMD created with ID: ' . $emd->id);
                    ...
                    Log::info('Bank Transfer created with ID: ' . $bt->id);

                    $tender = ($emd->tender_id != '00') ? TenderInfo::where('id', $emd->tender_id)->first() : null;

                    // If Tender Fee Applicable
                    switch ($request->tf_consent) {
                        case '1':
                            $popTenderFee = $this->createPopTenderFee($request, $emd, $tender);
                            Log::info('Payment on Portal Tender Fee Created: ', ['id' => $popTenderFee->id]);
                            $this->popTenderFeeMail($popTenderFee->id);
                            break;
                        case '2':
                            $btTenderFee = $this->createBtTenderFee($request, $emd, $tender);
                            Log::info('Bank Transfer Tender Fee Created: ', ['id' => $btTenderFee->id]);
                            $this->btTenderFeeMail($btTenderFee->id);
                            break;
                        case '3':
                            $ddTenderFee = $this->createDdTenderFee($request, $emd, $tender);
                            Log::info('Demand Draft Tender Fee Created: ', ['id' => $ddTenderFee->id]);
                            $file = $ddTenderFee->chqRec->generated_pdfs ? json_decode($ddTenderFee->chqRec->generated_pdfs, true) : [];
                            $this->ddTenderFeeMail($ddTenderFee->id, $file);
                            break;
                    }

                    $this->timerService->startTimer($bt, 'bt_acc_form', 3);
                    // Update tender status if needed
                    if ($emd->tender_id != '00') {
                        $tender = TenderInfo::where('tender_no', $emd->tender_no)->first();
                        if ($tender) {
                    ...
                    // Send mail
                    $mailSent = $this->bankTransferMail($emd->id);
                    DB::commit();
                    if ($mailSent) {
            ...
            Log::error('requestBt() exception: ' . $e->getMessage());
            return redirect()->route('emds.index')->with('error', $e->getMessage());
        }
    }

    public function requestPop(Request $request)
    {
        if ($request->instrument_type != '6') {
            return redirect()->route('emds.index')->with('error', 'Something went wrong!');
        }
        if ($request->method() == 'GET') {
            $id = base64_decode($request->tender_no);
            $data['tender'] = TenderInfo::where('id', $id)
                ...
                ->where('deleteStatus', '0')->first();
            if (!$data['tender']) {
                return redirect()->route('emds.index')->with('error', 'Tender not found.');
            }
            if (!$data['tender']->emd) {
                return redirect()->route('emds.index')->with('error', 'EMD amount not set.');
            }
            $data['instrumentType'] = $this->instrumentType;
            $data['instrument_type'] = $request->instrument_type;
            return view('emds.create-pop', $data);
        }
        if ($request->method() == 'POST') {
            Log::info('Creating Bank Transfer for EMD, user: ' . Auth::user()->name);
            Log::info('Request data: ' . json_encode($request->all()));
            ...
            try {
                ...
                Log::info('Bank Transfer created with ID: ' . $pop->id);
                ...
                $this->timerService->startTimer($pop, 'pop_acc_form', 3);
                // Update tender status if needed
                if ($emd->tender_id != '00') {
                    $tender = TenderInfo::where('tender_no', $emd->tender_no)->first();
                    if ($tender) {
    ...
    public function storeBasic($details)
    {
        try {
            $emd = new Emds();
            $emd->tender_id = $details['tender_id'] ?? '00';
            $emd->tender_no = $details['tender_no'] ?? 'NA';
            $emd->due_date = $details['due_date'];
            $emd->type = $details['tender_id'] == '0' ? 'Other Than TMS' : 'TMS';
            $emd->project_name = $details['project_name'];
            $emd->instrument_type = $details['instrument_type'];
            $emd->requested_by = $details['requested_by'];
            $emd->save();
            return $emd;
        } catch (\Throwable $e) {
    ...
    // --- Helper methods for tender fee creation ---
    private function createBtTenderFee($request, $emd, $tender)
    {
        $validated = $request->validate([
            'tf_amount' => 'required|numeric|min:0',
            'tf_purpose' => 'required|string|max:255',
            'tf_bt_acc_name' => 'required|string|max:255',
            'tf_bt_acc' => 'required|string|max:50',
            'tf_bt_ifsc' => 'required|string|max:11',
        ]);

        try {
            return BtTenderFee::create([
                'tender_id' => $tender ? $tender->id : '0',
                'emd_id' => $emd->id,
                'type' => $tender ? 'TMS' : 'Other Than TMS',
                'requested_by' => $request->requested_by,
                'tender_name' => $tender ? $tender->tender_name : $request->project_name,
                'due_date' => $tender && $tender->due_date ? $tender->due_date : $request->due_date,
                'purpose' => $validated['tf_purpose'],
                'account_name' => $validated['tf_bt_acc_name'],
                'account_number' => $validated['tf_bt_acc'],
                'ifsc' => $validated['tf_bt_ifsc'],
                'amount' => $validated['tf_amount'],
            ]);
        } catch (\Exception $e) {
            Log::error('Error creating BT Tender Fee: ' . $e->getMessage());
            return false;
        }
    }

    private function createDdTenderFee($request, $emd, $tender)
    {
        $request->validate([
            'tf_amount' => 'required|numeric|min:0',
            'tf_dd_needs' => 'required|string|in:due,24,36,48',
            'tf_purpose_of_dd' => 'required|string|max:255',
            'tf_in_favour_of' => 'required|string|max:255',
            'tf_dd_payable_at' => 'required|string|max:255',
            'tf_courier_address' => 'required|string',
            'tf_courier_deadline' => 'required|numeric|min:1',
        ]);

        try {
            $draft = new DdTenderFee();
            $draft->tender_id = $tender ? $tender->id : '0';
            $draft->emd_id = $emd->id;
            $draft->type = $tender ? 'TMS' : 'Other Than TMS';
            $draft->requested_by = $request->requested_by;
            $draft->tender_name = $tender ? $tender->tender_name : $request->project_name;
            $draft->due_date_time = $tender && $tender->due_date ? $tender->due_date : $request->due_date;
            $draft->dd_needed_in = $request->tf_dd_needs;
            $draft->in_favour_of = $request->tf_in_favour_of;
            $draft->dd_payable_at = $request->tf_dd_payable_at;
            $draft->dd_amount = $request->tf_amount;
            $draft->purpose_of_dd = $request->tf_purpose_of_dd;
            $draft->courier_address = $request->tf_courier_address;
            $draft->delivery_date_time = $request->tf_courier_deadline;
            $draft->status = 'DD requested';
            $draft->save();

            $chq = new EmdCheque();
            $chq->emd_id = $emd->id;
            $chq->req_type = 'Tender Fee';
            $chq->dd_id = $draft->id;
            $chq->cheque_favour = $request->in_favour_of;
            $chq->cheque_amt = $request->tf_amount;
            $chq->cheque_date = date('Y-m-d');
            $chq->cheque_reason = 'DD';
            $chq->cheque_needs = $request->tf_dd_needs;
            $chq->status = 'DD requested';
            $chq->save();

            $dd = [];
            $dd['cheque_date'] = date('d-m-Y');
            $dd['cheque_amt'] = $request->tf_amount;
            $dd['cheque_favour'] = $request->in_favour_of;
            $pdfFiles = $this->pdfGenerator->generatePdfs('chqCret', $dd);
            $chq->generated_pdfs = json_encode($pdfFiles); // save as JSON array
            $chq->save();

            Log::info("Tender Fee Demand Draft ($draft->id and Cheque ($chq->id) created.");
            return $draft;
        } catch (\Exception $e) {
            Log::error("Tender Fee Demand Draft and Cheque creation error: " . $e->getMessage());
            return false;
        }
    }

    private function createPopTenderFee($request, $emd, $tender)
    {
        $validated = $request->validate([
            'tf_amount' => 'required|numeric|min:0',
            'tf_purpose' => 'required|string|max:255',
            'tf_portal' => 'required|string|max:255',
            'tf_is_netbanking' => 'required|string|in:yes,no',
            'tf_is_debit' => 'required|string|in:yes,no',
        ]);

        try {
            return PopTenderFee::create([
                'tender_id' => $tender ? $tender->id : '0',
                'emd_id' => $emd->id ?? throw new \Exception('EMD not found'),
                'type' => $tender ? 'TMS' : 'Other Than TMS',
                'tender_name' => $tender ? $tender->tender_name : $request->project_name,
                'due_date_time' => $tender && $tender->due_date ? $tender->due_date : $request->due_date,
                'purpose' => $request->tf_purpose,
                'portal_name' => $request->tf_portal,
                'netbanking_available' => $request->tf_is_netbanking,
                'bank_debit_card' => $request->tf_is_debit,
                'amount' => $request->tf_amount,
            ]);
        } catch (\Exception $e) {
            Log::error("Tender Fee creation error: " . $e->getMessage());
            return false;
        }
    }
    ...
    {
        try {
            $responsibilityService = new EmdResponsibilityService();
            $emd = Emds::find($emdId);

            $sender = match ($emd->tender_id) {
                '00' => Auth::user(),
                default => $emd->tender->users,
            };

            $team = $sender->team ?? 'DC';
            $cc = $this->cc($team);
            $to = $responsibilityService->getResponsibleUser(1);
            $toName = $to->name ?? 'Shivani';

            $tlName = User::where('team', $team)
                ->where('role', 'team-leader')->value('name');
            $dd = EmdDemandDraft::where('emd_id', $emd->id)->firstOrFail();

            $signature = $responsibilityService->getSignatureForUser($sender);

            $tenderFee = DdTenderFee::where('emd_id', $emd->id)->first();

            $data = [
                'purpose' => $dd->dd_purpose,
                'partyName' => 'Yourself for DD',
            ...
            Mail::mailer($mailer)->to($to)->cc($cc)
                ->send(new DdCreatedMail($data));
            // Mail::mailer($mailer)->to('gyanprakashk55@gmail.com')
            //     ->send(new DdCreatedMail($data));

            Log::info("DD Creation Mail Sent");
    ...
    public function bankTransferMail($emdId)
    {
        try {
            ...
            $team = $sender->team ?? 'DC';
            $cc = $this->cc($team);
            $to = $responsibilityService->getResponsibleUser(5);
            $toName = $to->name ?? 'Shivani';
            ...
            $data = [
                ...
                'dueDateTime' => $emd->due_date ? date('d-m-Y h:i A', strtotime($emd->due_date)) : 'NA',
                'bt_acc' => $bt->bt_acc,
                'bt_ifsc' => $bt->bt_ifsc,
    ...
    public function popMail($emdId)
    {
        try {
            ...
            $team = $sender->team ?? 'DC';
            $cc = $this->cc($team);
            $to = $responsibilityService->getResponsibleUser(6);
            $toName = $to->name ?? 'Shivani';
            ...
            $data = [
                ...
                'portal' => $pop->portal,
                'purpose' => $pop->purpose,
                'dueDate' => $emd->due_date ? date('d-m-Y h:i A', strtotime($emd->due_date)) : 'NA',
    ...
    public function chequeMail($emdId, $pdfs)
    {
        try {
            ...
            $team = $sender->team ?? 'DC';
            $cc = $this->cc($team);
            $to = $responsibilityService->getResponsibleUser(3);
            $toName = $to->name ?? 'Shivani';
            ...
            $signature = $responsibilityService->getSignatureForUser($member);
    ...
    public function bgCreatedMail($bgId, $pdfs)
    {
        try {
            ...
            $member = match ($emd->tender_id) {
                0 => User::where('name', 'LIKE', "%$emd->requested_by")->first(),
                default => $emd->tender->users,
            };

            $team = $sender->team ?? 'DC';
            $cc = $this->cc($team);
            $to = $responsibilityService->getResponsibleUser(4);
            $toName = $to->name ?? 'Shivani';

            $tlName = User::where('team', $team)->where('role', 'team-leader')->value('name');
            ...
            MailHelper::configureMailer($member->email, $member->app_password, $member->name);
            // MailHelper::configureMailer('aasfa@volksenergie.in', 'mhlvvvqhqudecici', 'Aasfa');
            $mailer = Config::has('mail.mailers.dynamic') ? 'dynamic' : 'smtp';
    ...
    public function btTenderFeeMail($id)
    {
        try {
            $responsibilityService = new EmdResponsibilityService();
            $bt = BtTenderFee::find($id);

            $sender = match ($bt->tender_id) {
                '00' => Auth::user(),
                default => $bt->tender->users,
            };
            ...
            $data = [
                'id' => $bt->id,
                'purpose' => $bt->purpose,
                'tenderNo' => $bt->tender->tender_no ?? 'NA',
                'tenderName' => $bt->tender_name,
                'dueDateTime' => $bt->due_date ? date('d-m-Y h:i A', strtotime($bt->due_date)) : 'NA',
                'bt_acc' => $bt->account_number,
                'bt_ifsc' => $bt->ifsc,
                'bt_acc_name' => $bt->account_name,
                'amount' => $bt->amount,
                'tlName' => $tlName,
                'toName' => $toName,
                'sign' => $signature,
                'utr' => route('tender-fees.index'),
            ];
            Log::info("BT Tender Fee Mail Data: " . json_encode($data));
            ...
            Mail::mailer($mailer)->to($to)->cc($cc)
                ->send(new BankTransferMail($data));
            // Mail::mailer($mailer)->to('gyanprakashk55@gmail.com')
            //     ->send(new BtTenderFeeMail($data));

            Log::info("Mail sent successfully for Tender Fee ID: $id");
            return response()->json(['success' => true]);
        } catch (\Throwable $th) {
            Log::error("BT Tender Fee Mail Error: " . $th->getMessage());
            return response()->json(['success' => false, 'error' => $th->getMessage()]);
        }
    }

    public function popTenderFeeMail($id)
    {
        try {
            $responsibilityService = new EmdResponsibilityService();
            $pop = PopTenderFee::find($id);

            $sender = match ($pop->tender_id) {
                '00' => Auth::user(),
                default => $pop->tender->users,
            };
            ...
            $data = [
                'toName' => $toName,
                'tender_no' => $pop->tender->tender_no ?? 'NA',
                'tender_name' => $pop->tender_name,
                'portal' => $pop->portal_name,
                'purpose' => $pop->purpose,
                'dueDate' => $pop->due_date_time ? date('d-m-Y h:i A', strtotime($pop->due_date_time)) : 'NA',
                'netbanking' => $pop->netbanking_available,
                'debit' => $pop->bank_debit_card,
                'amount' => $pop->amount,
                'assignee' => $sender->name,
                'tlName' => $tlName,
                'sign' => $signature,
            ...
            Log::info("POP Tender Fee Mail Data: " . json_encode($data));
            ...
            Mail::mailer($mailer)->to($to)->cc($cc)
                ->send(new PopCreatedMail($data));
            // Mail::mailer($mailer)->to('gyanprakashk55@gmail.com')
            //     ->send(new PopTenderFeeMail($data));

            return response()->json(['success' => true]);
        } catch (\Throwable $th) {
            Log::error("POP Tender Fee Mail Error: " . $th->getMessage());
            return response()->json(['success' => false, 'error' => $th->getMessage()]);
        }
    }

    public function ddTenderFeeMail($id, $pdfs)
    {
        try {
            $responsibilityService = new EmdResponsibilityService();
            $dd = DdTenderFee::find($id);

            $sender = match ($dd->tender_id) {
                '00' => Auth::user(),
                default => $dd->tender->users,
            };
            ...
            $data = [
                ...
                'tlName' => $tlName,
                'toName' => $toName,
                'payableAt' => $dd->dd_payable_at,
                'courierAddress' => $dd->courier_address,
                'sign' => $signature,
                'pdfs' => $pdfs,
            ];
            Log::error("DD Created Mail Data: " . json_encode($data));
