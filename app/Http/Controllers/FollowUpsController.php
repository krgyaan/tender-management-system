...
class FollowUpsController extends Controller
{
    ...
    {
        $followups = FollowUps::with('followPerson', 'creator', 'assignee')->get();
                ...
                foreach ($request->fp['name'] as $key => $value) {
                    if (!empty($value) || !empty($request->fp['phone'][$key]) || !empty($request->fp['email'][$key])) {
                        ...
                        if (!$existingDir) {
                            ...
                            Log::warning("Duplicate entry found in Clintdirectory. Email: " . $request->client['email'][$key] . ", Phone: " . $request->client['phone'][$key]);
                        }
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'start_from' => 'required|date|after_or_equal:' . date('Y-m-d'),
    ...
    public function followupMailllll($id)
    {
        try {
            ...
            $fup = FollowUpPersons::where('follwup_id', $id)->get();
            ...
            if ($start_date->lte($today)) {
                ...
                $message = (new Swift_Message())
                    ...
                    ->setBody((new FollowupPersonMail($data))->render(), 'text/html');
            ...
            Log::error('Error during followupMail process test');
                ...
                if ($fu->emd_id) {
                    ...
                    switch ($ins) {
                        ...
                        case 4:
                            $data['for'] = $fu->followup_for;
                            $data['name'] = $fu->party_name;
                            $data['tenderNo'] = $fu->bg->emd->tender->tender_no;
                            $data['projectName'] = $fu->bg->emd->project_name;
                            $data['status'] = $fu->bg->emd->tender->statuses->name;
                            $data['amount'] = format_inr($fu->bg->bg_amount);
